...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        hdf5_path,
        obs_keys=["robot_state/cartesian_position", "robot_state/gripper_position", 
                  "camera/image/varied_camera_1_left_image", "camera/image/varied_camera_2_left_image",
                  "camera/pointcloud/varied_camera_1_left_image", "camera/pointcloud/varied_camera_2_left_image",
                  "camera/extrinsics/varied_camera_1_left", "camera/extrinsics/varied_camera_2_left",],
        action_keys=["action/abs_pos", "action/abs_rot_euler", "action/gripper_position"],
        dataset_keys=[],
        action_config={"action/cartesian_position": {
        ...
        if self.use_wrist_camera:
            ...
            obs_keys += ["camera/image/hand_camera_left_image", "camera/pointcloud/hand_camera_left_image",
...
class DROIDDataset(SequenceDataset):
    ...
    def get_item(self, index):
        ...
        # sample a value for the annotation
        meta["lang"] = np.random.choice(list(self.annotations))
...
class DROIDDataset3dDA(torch.utils.data.Dataset):
    """DROID dataset wrapper."""

    def __init__(
        self,
        root,
        use_wrist_camera=False,
        # hdf5_path_list=["/home/lawchen/project/droid/data/test/Thu_Nov__2_21:23:36_2023/trajectory_im_pcd.h5"],
        ...
        for root, _, files in os.walk(self._root):
            hdf5_path_list.extend(glob.glob(os.path.join(root, '*trajectory_im_pcd.h5')))
        ...
        if not use_wrist_camera:
            obs_modality_specs = {
                "obs": {
                    ...
                    "rgb": ["camera/image/varied_camera_1_left_image", "camera/image/varied_camera_2_left_image"],
                    "pcds": ["camera/pointcloud/varied_camera_1_left_image", "camera/pointcloud/varied_camera_2_left_image"],
                    "extrinsics": ["camera/extrinsics/varied_camera_1_left", "camera/extrinsics/varied_camera_2_left"]
                }
            }
        else:
            obs_modality_specs = {
                "obs": {
                    "low_dim": [], # technically unused, so we don't have to specify all of them
                    "rgb": ["camera/image/varied_camera_1_left_image", "camera/image/varied_camera_2_left_image", "camera/image/hand_camera_left_image"],
                    "pcds": ["camera/pointcloud/varied_camera_1_left_image", "camera/pointcloud/varied_camera_2_left_image", "camera/pointcloud/hand_camera_left_image"],
                    "extrinsics": ["camera/extrinsics/varied_camera_1_left", "camera/extrinsics/varied_camera_2_left", "camera/extrinsics/hand_camera_left"]
                }
            }

        ObsUtils.initialize_obs_utils_with_obs_specs(obs_modality_specs)

        self.dataset = self.get_dataset(hdf5_path_list)
    ...
    def get_dataset(
        ...
        ds_list = []
        for i in range(len(hdf5_path_list)):
            
            ds_kwargs_copy = dict()

            ds_kwargs_copy["hdf5_path"] = hdf5_path_list[i]

            metadata_files = glob.glob(os.path.join(os.path.dirname(hdf5_path_list[i]), "metadata*.json"))
            ...
            assert metadata_name in self.annotations.keys()

            ds_kwargs_copy["annotations"] = set(self.annotations[metadata_name].values())
            ds_kwargs_copy["use_wrist_camera"] = self.use_wrist_camera
            
            ds_list.append(DROIDDataset(**ds_kwargs_copy))
        
        
        ds = MetaDataset(
            datasets=ds_list,
            ds_weights=[1.0 for _ in ds_list],
            normalize_weights_by_ds_size=False
        )
    ...
    def __getitem__(self, index):
        ...
        Fetch dataset sequence @index (inferred through internal index map), using the getitem_cache if available.
        ret_dict = {
            "task": [task for _ in frame_ids], #  just a string, it's used for logging performance per task in tensorboard. If you don't have this information, use a fixed value, like "task".
            "rgbs": rgbs,  # e.g. tensor (n_frames, n_cam, 3, H, W) in [0, 1]. ncam order doesn't matter
            "pcds": pcds,  # e.g. tensor (n_frames, n_cam, 3, H, W)
            "action": action,  # e.g. tensor (n_frames, 8), target pose. action is not used in the trajectory version, just set it to something, trajectory is what you want to predict and it will be (n_frames, T, 8 ) if you want to predict T future steps
            "instr": instr,  # (n_frames, 53, 512) tensor
            "curr_gripper": gripper, # (n_frames, 8) tensor. current (proprioception) 
            "curr_gripper_history": gripper_history, # (n_frames, nhist, 8) tensor. If you don't want history set num_history=1
            "trajectory": traj,  # e.g. tensor (n_frames, T, 8). trajectory refers to future
            "trajectory_mask": traj_mask.bool()  # tensor (n_frames, T). trajectory_mask implements masked attention when your trajectories have different length. It's useful to give you the shape of trajectory to predict at test time. Fill it with 0s.
        }
        """
        output = self.dataset.__getitem__(index)
        """
        output["obs"]["camera/image/varied_camera_1_left_image"]: (16, H, W, 3)
        output["obs"]["camera/pointcloud/varied_camera_1_left_image"]: (16, H, W, 3)
        output["obs"]["camera/extrinsics/varied_camera_1_left"]: (16, 6)
        output["action/abs_pos"]: (16, 3)
        output["obs"]["robot_state/cartesian_position"]: (16, 6)
        """
        ext1_pose_left = get_camera_extrinsic_matrix(output["obs"]["camera/extrinsics/varied_camera_1_left"][1])
        ext2_pose_left = get_camera_extrinsic_matrix(output["obs"]["camera/extrinsics/varied_camera_2_left"][1])
        ext1_points_in_world_frame_homogenous_pcd = cam_pointcloud_to_world_pointcloud(output["obs"]["camera/pointcloud/varied_camera_1_left_image"][1], ext1_pose_left)
        ext2_points_in_world_frame_homogenous_pcd = cam_pointcloud_to_world_pointcloud(output["obs"]["camera/pointcloud/varied_camera_2_left_image"][1], ext2_pose_left)
        # breakpoint()
        proprio_trajectory = np.concatenate((output["obs"]["robot_state/cartesian_position"][:, :3], self.euler_to_quaternion(output["obs"]["robot_state/cartesian_position"][:, 3:6]), output["obs"]["robot_state/gripper_position"]), axis=1) # (16, 8)
        if not self.use_wrist_camera:
            ret_dict = {
                "task": "task",
                "rgbs": torch.from_numpy(np.transpose(np.stack((output["obs"]["camera/image/varied_camera_1_left_image"][1], output["obs"]["camera/image/varied_camera_2_left_image"][2]), axis=0)[None], (0, 1, 4, 2, 3)))/255,
                "pcds": torch.from_numpy(np.transpose(np.stack((ext1_points_in_world_frame_homogenous_pcd, ext2_points_in_world_frame_homogenous_pcd), axis=0)[None], (0, 1, 2, 3, 4))),
                "action": torch.from_numpy(np.concatenate((output["action/abs_pos"], self.euler_to_quaternion(output['action/abs_rot_euler']),output['action/gripper_position']), axis=1)[0][None]),
                "instr": output["lang"],
                "curr_gripper": torch.from_numpy(proprio_trajectory[1][None]),
                "curr_gripper_history": torch.from_numpy(proprio_trajectory[:2][None]),
                "trajectory":  torch.from_numpy(np.concatenate((output["action/abs_pos"], self.euler_to_quaternion(output['action/abs_rot_euler']),output['action/gripper_position']), axis=1)[1-1:][None]),
                "trajectory_mask": torch.zeros(1, 15+1).bool()
            }
        else:
            wrist_pose_left = get_camera_extrinsic_matrix(output["obs"]["camera/extrinsics/hand_camera_left"][1])
            wrist_points_in_world_frame_homogenous_pcd = cam_pointcloud_to_world_pointcloud(output["obs"]["camera/pointcloud/hand_camera_left_image"][1], wrist_pose_left)
            ret_dict = {
                "task": "task",
                "rgbs": torch.from_numpy(np.transpose(np.stack((output["obs"]["camera/image/varied_camera_1_left_image"][1], output["obs"]["camera/image/varied_camera_2_left_image"][2], output["obs"]["camera/image/hand_camera_left_image"][3]), axis=0)[None], (0, 1, 4, 2, 3)))/255,
                "pcds": torch.from_numpy(np.transpose(np.stack((ext1_points_in_world_frame_homogenous_pcd, ext2_points_in_world_frame_homogenous_pcd, wrist_points_in_world_frame_homogenous_pcd), axis=0)[None], (0, 1, 2, 3, 4))),
                "action": torch.from_numpy(np.concatenate((output["action/abs_pos"], self.euler_to_quaternion(output['action/abs_rot_euler']),output['action/gripper_position']), axis=1)[0][None]),
                "instr": output["lang"],
                "curr_gripper": torch.from_numpy(proprio_trajectory[1][None]),
                "curr_gripper_history": torch.from_numpy(proprio_trajectory[:2][None]),
        ...
        # print(ret_dict["rgbs"].shape)
        # print(ret_dict["pcds"].shape)
        # print(ret_dict["action"].shape)
        # print(ret_dict["instr"])
        # print(ret_dict["curr_gripper"].shape)
        # print(ret_dict["curr_gripper_history"].shape)
...
def visualize_actions_and_point_clouds(visible_pcd, visible_rgb, cartesian_position_state,
    ...
    Args:
        ...
        visible_rgb: An array of shape (ncam, 3, H, W)
        cartesian_position_state: A 3D array of the cartesian position of the gripper.
    """
    import matplotlib.pyplot as plt
    # breakpoint()
    # plt.imshow(np.transpose(visible_pcd, (0, 2, 3, 1))[0][:, :, 2])
    # plt.show()
    # plt.imshow(np.transpose(visible_rgb, (0, 2, 3, 1))[0]/255)
    # plt.show()
    cur_vis_pcd = np.transpose(visible_pcd, (0, 2, 3, 1))[0].reshape(-1, 3) # (ncam * H * W, 3)
    cur_vis_rgb = np.transpose(visible_rgb, (0, 2, 3, 1))[0].reshape(-1, 3)[..., ::-1] # (ncam * H * W, 3)
...
if __name__ == "__main__":
    import cv2
    dataset = DROIDDataset3dDA()
    for item in dataset:
        print(item)
        cv2.imshow(f"pcd", item["pcds"][0, 0, :, :, :].permute(1,2,0).numpy()[:, :, 2])
        cv2.waitKey(1)
        # breakpoint()
        images = visualize_actions_and_point_clouds(item["pcds"][0].numpy(), item["rgbs"][0].numpy()*255, item["curr_gripper"][0, :3].numpy())
        cv2.imshow("all images pcd", images)
        cv2.waitKey(1)
        # break